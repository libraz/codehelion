...
「ソース上に写しが何個あるか」と「バイナリ上に実体が何個あるか」は別の軸で、codehelion の探索モデルにあるのは前者だけです。ある実測では、回収できた 2,726 バイトのうち 1,730 バイト（63%）が、ソース上は 1 本のテンプレートでありながら、述語のクロージャ型が呼び出し箇所ごとに違うためバイナリ上では 14 個の別々の実体になっていたものでした。ソース上の写しは 1 つなので、この多重度を述べるクローングループは存在しません。同じツリーで codehelion が実際に報告したグループが説明したのは、残りの 996 バイト、つまり小さいほうです。サイズが写しの数ではなく実体化の数で決まっているバイナリについては、これはそれを測るツールではありません。
