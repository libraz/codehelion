...
**成果物検査はシンボルに依存します。** strip 済みのバイナリからはほとんど何も得られないため、strip していないビルドか、identity を検証できるデバッグ情報を渡してください。レジスタと即値の違いを吸収する重複検出は x86 でのみ実装しており、ほかのアーキテクチャではバイト単位で一致する重複だけを検出します。
