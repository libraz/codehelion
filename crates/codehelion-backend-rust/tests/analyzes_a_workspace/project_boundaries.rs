...
/// A standard optional and a standard fallible are one shape, whatever
/// language they are written in. Two backends that categorise them differently
/// lower the score of exactly the cross-language clones the category exists to
/// find, so the answer is pinned here rather than left to how each library
/// happens to declare them.
#[test]
fn a_standard_optional_and_a_standard_fallible_are_one_category() {
    let ir = stamped();
    assert_eq!(category_of(&ir, "longest"), TypeCategory::Enumeration);
    assert_eq!(
        category_of(&ir, "preserve_result"),
...
/// A project whose workspace manifest sits above the boundary an untrusted
/// request set is declined rather than read.
///
/// The workspace manifest names the members, the profiles and the patched
/// dependencies a member is compiled with, so answering about the member is
/// answering about that file too. Both halves are asserted from one tree: the
/// same unit under a boundary that holds the whole project is analysed, which
/// is what makes the refusal above about the boundary rather than about the
/// project.
#[test]
fn a_workspace_manifest_above_the_boundary_leaves_a_member_unanalysable() {
    let directory = tempfile::tempdir().expect("temporary workspace");
    let outer = directory.path();
    let member = outer.join("inner");
    ...
    let mut helper = helper();
    let confined = helper
        .analyze_with_command_and_boundary(&unit, None, Some(&member), &[Capability::Types])
        .expect("the helper should answer");
    let explained = said_while_refusing(&helper);
    let whole = helper
        .analyze_with_command_and_boundary(&unit, None, Some(outer), &[Capability::Types])
        .expect("the helper should answer");
    helper.shutdown().expect("the helper should stop cleanly");
    ...
    // The reason on the wire stands for several situations. Which one this was
    // reaches a run through what the helper said while refusing, so a helper
    // that refuses in silence leaves that path with nothing to carry.
    assert!(
        explained
            .iter()
            .any(|line| line.contains("outside the directory this request confined reading to")),
        "the refusal explained itself nowhere: {explained:?}"
    );
    assert!(
        matches!(whole, Analysis::Done(_)),
        "the same unit is readable when the boundary holds its workspace: {whole:?}"
    );
}

/// What the helper printed while refusing the unit just asked about.
///
/// The stream is read by a thread of its own, so a line written before the
/// answer can arrive here after it. Waited for rather than sampled once: the
/// helper has written it, and what is being allowed for is the reader being
/// scheduled.
fn said_while_refusing(helper: &Helper) -> Vec<String> {
...
/// A file outside the boundary is declined before any project is looked for,
/// so nothing above the boundary is read on the way to refusing it.
#[test]
fn a_file_outside_the_boundary_is_not_analysed() {
    let unit = unit("plain", "ledger", "ledger");
    let elsewhere = tempfile::tempdir().expect("a directory holding no part of the project");
    let mut helper = helper();
    let answer = helper
        .analyze_with_command_and_boundary(
            &unit,
            None,
            Some(elsewhere.path()),
            &[Capability::Types],
