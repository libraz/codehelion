...
fn artifact_container_facts_reach_json_text_and_csv_without_raw_data() {
    ...
    assert!(
        csv.lines()
            ...
            .unwrap()
            .ends_with(
                "section,executable,module,duplicated_bytes_normalized,estimated_duplicated_bytes,attribution_basis"
            )
    );
...
fn an_archive_with_repeated_member_identities_reports_no_reachability() {
    ...
    assert!(
        String::from_utf8(text)
            .unwrap()
            .contains("dead code: unavailable (no resolved exported root set)")
    );
}
