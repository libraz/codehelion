...
## seam はクローングループとは別種の作業単位

クローングループが名指しするのは繰り返されているコードです。[seam](seam-tracking.md) が名指しするのは一緒に動かす必要のあるパスであり、レポートは、一緒に動かさなかったことが何を要したかを述べます。

```text
seams: frontend-c-cpp 12 asymmetric changes, 7 breaches (last 6e014d86), 1,553 findings
```

並べる軸にするのは breach の件数です。非対称変更とは、メンバーの一部だけを触って残りをそのままにしたコミットのことで、そのうち相当数は正しい変更です。breach はそのあとに、触らなかったメンバーへの `fix:` が続いたものであり、そのリポジトリは非対称の代償をすでに一度払っています。隣の findings は、同じパスの中でスキャンが見つけた重複の件数です。ここで 2 種類の作業単位が交わります。breach していて finding も抱えている seam は、このループがそのまま手を付けられる対象です。
...
を走らせると、目の前のファイルと一緒に動かす必要のあるメンバーが分かります。
