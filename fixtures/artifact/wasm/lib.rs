...
#[unsafe(no_mangle)]
#[inline(never)]
// The reduced fixture models callers converging on duplicate_left, allowing
// the now-redundant public entry point and its generic instantiation to go.
#[cfg(not(deduplicated))]
pub extern "C" fn duplicate_right(value: i32) -> i32 {
    let mut value = duplicate_template(value);
    value = value.wrapping_add(1);
    value = value.rotate_left(3);
    value = value.wrapping_mul(5);
